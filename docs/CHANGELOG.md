...
### Analysis layer review fixes — the panels can no longer state a claim the data does not support

**Why:** The whole-branch review of feat/analysis-layer found one Critical, nine Important and ten smaller defects, nearly all of the same shape: a panel stating a conclusion the underlying data cannot support, or a guard that passes without ever executing. All fixed in one wave. The Critical: evaluate() hardcoded 'lower is better', so an intervention that doubled adoption from 7 active days to 14 — exactly what such a change aims at — reported not-supported. interventions.json now takes an optional "direction": "down" | "up", defaulting to down so every existing declaration keeps its meaning, and the spec gains the subsection it was missing (it named a factor and never a direction while listing only reduction-flavoured verdicts). Also: the after-window could end ON today, so a verdict computed from a still-accumulating day was final and freezable (now strictly before, per the spec's own reproducibility rule); provisional lost to underpowered, so a mid-flight window was reported as thin rather than as unfinished, with the days-remaining figure computed on an unreachable branch; expectedShift exempted a whole dimension where the spec says category, so declaring an opus->sonnet swap laundered an unannounced switch to a third model; a second intervention on the SAME DAY was silently not a confound, the one case the self-skip dropped; and the confound shares spanned every tool while factorsFor counts only turn-capable ones, so a Codex-driven mix swing could confound a comparison no Codex cost entered. Two labelling fixes reach numbers a reader sees: driver decomposition built its compared period from all tools, advertising 2026-04-27 - 2026-07-21 for a before-half whose Claude data begins 2026-06-11 (six weeks of period contributing nothing, verified on the live corpus; now 2026-06-11 - 2026-07-22), and coverage now excludes the zero-call daily rows turn counting introduced — 10 exist on the live corpus, e.g. 2026-07-19 with 28 turns and 0 calls — which could otherwise move the burn-rate denominator and push the intervention coverage floor earlier, the unsafe direction. oracleSkipped is now rendered distinctly from orderSensitive: when a factor is zero the order-independence check cannot run, and 'we never checked' previously displayed identically to 'we checked and it is fine'. Spec guard 5 (variance: a daily series with n, mean and median) stays deferred, but visibly — the plan's claim that it was 'computed in factorsFor' was false and is corrected, and it is now a real BACKLOG item. Real-corpus verification after the wave: 1658 turns, 53791 Claude requests, 32.44 requests/turn (32.42 at ship), subagent turns exactly 0, 121 of 121 codex/opencode rows carrying userTurns null and none carrying 0, zero sessions breaching the requests/turn >= 1 floor. 185 tests pass.

### Driver decomposition and intervention evaluation panels

**Why:** Two panels close out the analysis layer: driver decomposition splits cost into activeDays x turnsPerActiveDay x requestsPerTurn x tokensPerRequest x pricePerToken (day-sliced, all-four-bucket Tokens/Request so the identity multiplies back out to real cost) so a bill change can be pinned on one factor instead of argued about qualitatively; intervention evaluation compares matched before/after windows for a pre-registered expect (declared before the result is seen — the whole mechanism that makes a verdict mean anything), checks model/project/subagent/cache-mix confounds, and reports supported/not-supported/confounded/underpowered/provisional/pending/refused with reasons attached. interventions.json and interventions.results.json are both gitignored and never leave the machine. Real-corpus verification at ship: 1652 turns, 53563 requests, 32.42 requests/turn, agreeing with the 32.81 measured at the 2026-09-03 design pass, subagent turns exactly 0, zero sessions breaching the requests/turn >= 1 floor, turnCoverage.share 0.951. AGENTS.md now records that this panel's Tokens/Request (four buckets) and PerCallTrend's per-call context (three buckets, excludes output) are deliberately different measures. Follow-up recorded in BACKLOG (Someday): use ~/.claude/history.jsonl as a direct deletion detector for the coverage guard, deliberately deferred to avoid a new data source mid-build.
