...
export const GLOSSARY = {
  ...
  'driver decomposition': {
    ...
    body: 'Splits a change in spend between the five things that can cause it, so a bigger bill can be traced to using the tool more, sending bigger prompts, or paying a higher rate. Each factor is measured holding the others still, and the five contributions add up to the total change exactly. Because each factor is measured in turn, the order can change how the credit is shared out, so a second order-independent method runs behind the scenes as a check and the panel says when the two disagree. That check is one yes-or-no answer weighed against the largest contribution shown, so a small factor can be badly misattributed in relative terms without tripping it — and when any factor is zero the check cannot run at all, which the panel reports separately. Not checked is not the same as checked and sound.',
  },
  'intervention': {
    title: 'Intervention',
